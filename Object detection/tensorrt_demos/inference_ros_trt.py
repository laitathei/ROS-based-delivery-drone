...
def loop_and_detect(trt_yolo, conf_th, vis):
    ...
    while(True):
            ...
            for id, box in enumerate(boxes):
                if clss[id] == 1.0:
                    try:
                        ...
                        cv2.circle(detection.rgb_image,(centre_x,centre_y), 5, (0,100,200),cv2.FILLED)
                        cropped_img = detection.depth_image[centre_y-25:centre_y+25,centre_x-25:centre_x+25]
                        avg = np.mean(cropped_img)
                        cv2.putText(detection.rgb_image,"%.2f m" %(avg/1000),(left+10 ,top+20),cv2.FONT_HERSHEY_COMPLEX_SMALL,1,(0,128,0),3)
        ...
        if len(depth_data) != 0:
            ...
            left,top,right,bottom = detection.angle_data.regulation_box(box,detection.rgb_width,detection.rgb_height)

            cropped_img = detection.rgb_image[left:right,top:bottom]
            cropped_img = detection.detector.findPose(cropped_img,True)
            lmList = detection.detector.findPosition(cropped_img,True)
            Left_straight, Right_straight, Left_angle, Right_angle, Gesture =detection.angle_data.cal_angle(lmList)
            #depth_of_human,cx,cy = detection.angle_data.depth_calculation(lmList,detection.depth_array,left,top)
            ...
            detection.human_gesture_Publisher.publish(str(Gesture))
            try:
                ros_image = detection.bridge.cv2_to_imgmsg(cropped_img, "bgr8")
                detection.human_image_Publisher.publish(ros_image)
...
class object_detect:
    def __init__(self):
        ...
        self.human_image_Publisher = rospy.Publisher('/detected_human',Image,queue_size=1)
        self.human_gesture_Publisher = rospy.Publisher('/detected_human_gesture',String,queue_size=1)
