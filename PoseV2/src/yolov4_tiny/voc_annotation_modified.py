...
#   默认指向根目录下的VOC数据集
#-------------------------------------------------------#
DataSet_path  = 'yolov4_tiny/DataSet'
...
if __name__ == "__main__":
    ...
    if annotation_mode == 0 or annotation_mode == 2:
        ...
        for image_set in VOCdevkit_sets:
            image_ids = open(os.path.join(DataSet_path, 'ImageSets/%s.txt'%(image_set)), encoding='utf-8').read().strip().split()
            list_file = open(os.path.join(DataSet_path,'YOLOTrainList/%s.txt'%( image_set)), 'w', encoding='utf-8')
